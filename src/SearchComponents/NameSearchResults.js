...
const NameSearchResults = () => {
  ...
  const fetchSearchResults = async (page = 1) => {
    ...
    const apiUrl = `https://port-0-back-m341pqyi646021b2.sel4.cloudtype.app/cafes/search?name=${encodeURIComponent(
    ...
    try {
      const response = await fetch(
        `https://port-0-back-m341pqyi646021b2.sel4.cloudtype.app/cafes/search?name=${encodeURIComponent(
          requestData.cafe_name
        )}&limit=10&page=${page}`
      );
